...
class FinalVault:
    ...
    def render(self, surface):
        ...
        border_color = (60, 220, 110) if self.complete else (180, 80, 80)
        pygame.draw.rect(
        ...
        # Title
        title = title_font.render("FINAL LEVEL - MASTER VAULT", True, (255, 255, 255))
        ...
        progress_rect = progress.get_rect(
            topright=(self.room_x + self.room_width - 25, self.room_y + 23)
        )
        surface.blit(progress, progress_rect)

        # Objective
        objective_text = text_font.render(self._get_objective(), True, (195, 205, 215))
        surface.blit(objective_text, (70, 90))

        # Vault data panel
        data_panel = pygame.Rect(70, 125, 300, 150)
        pygame.draw.rect(surface, (8, 12, 20), data_panel)
        pygame.draw.rect(surface, (70, 105, 140), data_panel, 2)

        data_heading = heading_font.render("VAULT DATA", True, (80, 210, 255))
        surface.blit(data_heading, (90, 142))
        ...
        for line in data_lines:
            rendered = code_font.render(line, True, (205, 220, 230))
            surface.blit(rendered, (90, y))
            y += 23

        # Challenge panel
        challenge_panel = pygame.Rect(400, 125, 470, 150)
        pygame.draw.rect(surface, (10, 14, 23), challenge_panel)
        pygame.draw.rect(surface, (100, 110, 150), challenge_panel, 2)

        challenge_heading = heading_font.render("MASTER VAULT CHALLENGE", True, (235, 235, 240))
        surface.blit(challenge_heading, (420, 142))

        y = 180
        for line in self._get_code_lines():
            rendered = code_font.render(line, True, (100, 225, 255))
            surface.blit(rendered, (420, y))
            y += 24

        # Options
        options_heading = heading_font.render("SELECT THE CORRECT ACTION", True, (245, 245, 245))
        surface.blit(options_heading, (70, 315))
        ...
        for index, option in enumerate(self.options):
            ...
            if self.selected_option == index + 1:
                color = (70, 230, 120) if self._is_correct_option(index + 1) else (255, 100, 100)

            option_text = option_font.render(f"[{index + 1}] {option}", True, color)
            surface.blit(option_text, (85, option_y))
            option_y += 32

        # Terminal
        terminal_heading = heading_font.render("FINAL TERMINAL", True, (80, 210, 255))
        surface.blit(terminal_heading, (500, 315))

        pygame.draw.rect(surface, (5, 8, 14), (500, 355, 350, 42))
        pygame.draw.rect(surface, (80, 100, 125), (500, 355, 350, 42), 2)

        input_text = text_font.render(
            self.command if self.command else "Enter final value...",
            True, (230, 230, 235),
        )
        surface.blit(input_text, (512, 365))

        terminal_instruction = text_font.render(
            "Use the terminal when the challenge asks.",
            True, (165, 175, 190),
        )
        surface.blit(terminal_instruction, (500, 410))

        # Feedback
        if self.feedback:
            color = (70, 230, 120) if self.complete else (255, 110, 100)
            feedback = feedback_font.render(self.feedback, True, color)
    ...
    def handle_event(self, event):
        ...
        # Answer selection
        option_keys = {pygame.K_1: 1, pygame.K_2: 2, pygame.K_3: 3}
        if event.key in option_keys:
            ...
            if self._is_correct_option(selected):
                ...
                if self.stage < self.total_stages:
                    ...
                    if self.game:
                        self.game.score.add(150)   # big final bonus
            else:
                self.feedback = (
    ...
    def _get_objective(self):
        ...
        return objectives.get(self.stage, "Complete the Master Vault challenge.")
    ...
    def _get_options(self):
        ...
        options = {
            ...
            2: ["+", "*", "/"],
            3: ["continue", "return", "break"],
            4: ["vault['key']", "vault[0]", "vault.key"],
            5: ["parameter", "argument", "return value"],
            6: ["Ignore the error", "Use try/except", "Delete the function"],
        }
        return options.get(self.stage, [])
    ...
    def _submit_terminal(self):
        ...
        if self.stage not in (1, 2, 4, 6):
            self.feedback = "This challenge is solved using the answer panel."
            return
        ...
        try:
            ...
            if self.stage == 1:
                if value == self.clearance_level:
                    self.command_result = "Integer accepted: clearance verified."
                else:
                    self.command_result = "Wrong clearance level."
                    if self.game:
                        self.game.security.increase(10)

            elif self.stage == 2:
                if value == 42:
                    self.command_result = "Security code verified: 42."
                else:
                    self.command_result = "Incorrect security code."
                    if self.game:
                        self.game.security.increase(10)

            elif self.stage == 4:
                if value == self.inventory["access_code"]:
                    self.command_result = "Dictionary lookup verified: 42."
                else:
                    self.command_result = "Incorrect vault value."
                    if self.game:
                        self.game.security.increase(10)

            elif self.stage == 6:
                if value == 5:
                    self.command_result = "Input recovered safely."
                else:
                    self.command_result = "Recovery rejected the invalid value."

        except ValueError:
            self.command_result = "ValueError caught: invalid input handled safely."
            if self.stage == 6:
                self.feedback = "EXCEPTION HANDLED! try/except recovery works."
            if self.game:
                self.game.security.increase(5)
