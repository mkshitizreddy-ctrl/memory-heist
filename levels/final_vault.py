...
class FinalVault:
    ...
    def __init__(self, game=None):
        ...
        # Used for the function challenge.
        self.authorization_level = 5

        # Prevent duplicate score awards.
        self._stage_scores = set()
    ...
    def render(self, surface):
        """Draw the Master Vault and current challenge."""
        ...
        progress = text_font.render(
            ...
            f"{self.stage}/{self.total_stages}",
            True,
            (255, 215, 80),
        )
        ...
        if self._terminal_required():
            ...
            input_placeholder = (
                self.command
                if self.command
                else "Enter final value..."
            )

        else:

            input_placeholder = (
                "Terminal not required for this stage"
            )
        ...
        if self.feedback:
            ...
            color = (
                ...
                if self.complete
                    ...
                    (70, 230, 120)
                    if self.feedback.startswith("CORRECT")
                    else (255, 110, 100)
                )
            )
    ...
    def handle_event(self, event):
        ...
        if event.key in option_keys:
            ...
            if self._is_correct_option(selected):
                ...
                if (
                    self.game
                    and self.stage not in self._stage_scores
                ):
                    self.game.score.add(50)
                    self._stage_scores.add(self.stage)

                # Stage 6 completes the entire vault.
                if self.stage >= self.total_stages:
                    ...
                    if (
                        self.game
                        and not self._completion_awarded
                    ):
                        self.game.score.add(150)
                    ...
                    self.feedback = (
                        "CORRECT! "
                        "Next security challenge unlocked."
                    )
    ...
    def _get_objective(self):
        ...
        objectives = {
            ...
            1:
                "Use the correct data type for the clearance level.",

            2:
                "Choose the operator that generates the access code.",

            3:
                "Identify the Python loop control statement.",

            4:
                "Retrieve the correct value from the dictionary.",

            5:
                "Identify what a function sends back to its caller.",

            6:
                "Handle invalid input safely with exception handling.",
        }
    ...
    # TERMINAL
    def _terminal_required(self):
        """
        Return whether the current stage requires terminal input.

        Stage 6 is the actual exception-handling terminal challenge.
        """

        return self.stage == 6

    def _terminal_instruction(self):
        """Return terminal instructions for the current stage."""

        if self.stage == 6:
            return "Type an integer or invalid text, then press ENTER."

        return "This challenge is solved with the answer choices."

    def _submit_terminal(self):
        """
        Process terminal input and demonstrate exception handling.

        The final stage intentionally accepts invalid input so that
        the player can observe how ValueError is caught.
        """

        if self.stage != 6:
            self.feedback = (
                "Terminal is not required for this challenge."
            )
            return

        if not self.command:
            self.feedback = "Enter a value before pressing ENTER."
            return
        ...
        try:
            ...
            if value == self.clearance_level:
                ...
                self.command_result = (
                    "Integer accepted: clearance level verified."
                )

                self.feedback = (
                    "Correct integer input. "
                    "Now solve the exception challenge."
                )

                # Do not automatically complete the level from
                # terminal input. The player must still choose
                # try/except.

            else:

                self.command_result = (
                    f"Integer received: {value}"
                )

                self.feedback = (
                    "Input was valid, but the clearance "
                    "value is not 5."
                )
